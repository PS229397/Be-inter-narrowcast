...
<div x-show="getSelectedLeafComponent() === 'text'" class="flex h-full flex-col gap-3">
    ...
    <div class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-white/10 bg-[#24242a]">
        ...
        <div class="flex shrink-0 flex-wrap items-center gap-0.5 border-b border-white/10 px-2 py-1.5">
            <button type="button" x-on:mousedown.prevent x-on:click.stop="execTextCommand('bold')"
                class="rounded px-2 py-0.5 text-xs font-bold text-zinc-400 transition hover:bg-white/5 hover:text-white">B</button>
            <button type="button" x-on:mousedown.prevent x-on:click.stop="execTextCommand('italic')"
                class="rounded px-2 py-0.5 text-xs italic text-zinc-400 transition hover:bg-white/5 hover:text-white">I</button>
            <button type="button" x-on:mousedown.prevent x-on:click.stop="execTextCommand('underline')"
            ...
            <button type="button" x-on:mousedown.prevent x-on:click.stop="execTextCommand('formatBlock', 'h1')"
                class="rounded px-2 py-0.5 text-xs text-zinc-400 transition hover:bg-white/5 hover:text-white">H1</button>
            <button type="button" x-on:mousedown.prevent x-on:click.stop="execTextCommand('formatBlock', 'h2')"
                class="rounded px-2 py-0.5 text-xs text-zinc-400 transition hover:bg-white/5 hover:text-white">H2</button>
            <button type="button" x-on:mousedown.prevent x-on:click.stop="execTextCommand('formatBlock', 'p')"
                class="rounded px-2 py-0.5 text-xs text-zinc-400 transition hover:bg-white/5 hover:text-white">¶</button>
            <div class="mx-1 h-4 w-px bg-white/10"></div>
            <button type="button" x-on:mousedown.prevent x-on:click.stop="execTextCommand('justifyLeft')"
            ...
            <button type="button" x-on:mousedown.prevent x-on:click.stop="execTextCommand('justifyCenter')"
                class="rounded px-1.5 py-0.5 text-zinc-400 transition hover:bg-white/5 hover:text-white">
            ...
            <button type="button" x-on:mousedown.prevent x-on:click.stop="execTextCommand('justifyRight')"
                class="rounded px-1.5 py-0.5 text-zinc-400 transition hover:bg-white/5 hover:text-white">
            ...
            {{-- Font size --}}
            <select
                x-on:change="if ($event.target.value) { execTextCommand('fontSize', $event.target.value); $event.target.value = ''; }"
            ...
            <label class="flex cursor-pointer items-center gap-1 rounded px-1.5 py-0.5 text-zinc-400 transition hover:bg-white/5" title="Text color">
                ...
                <input type="color" value="#ffffff"
                    x-on:input="execTextCommand('foreColor', $event.target.value)"
        ...
        <div
            ...
            x-ref="textEditor"
            class="min-h-[120px] flex-1 overflow-y-auto break-words p-3 text-sm text-zinc-200 outline-none [overflow-wrap:anywhere] [&_*]:max-w-full [&_*]:break-words [&_*]:whitespace-normal [&>h1]:text-2xl [&>h1]:font-bold [&>h2]:text-lg [&>h2]:font-semibold"
            x-on:input="syncTextEditorContent()"
            x-effect="
